...
import { useMailUpdate } from "../../hooks/mail/useMailUpdate";
...
const DvPersuratan = () => {
  ...
  useEffect(() => {
    if (mail) {
      setJudul(mail.judul);
      setDeskripsi(mail.deskripsi);
      setKategori(mail.kategori);
      setTglUpload(mail.tgl_upload);
      setFile(null);
    }
  }, [mail]);

  const { mutate, isPending } = useMailUpdate();

  // Handle submit form
  const storeUpdateMail = async (e: FormEvent) => {
    e.preventDefault();
    ...
    mutate(
      {
        id: Number(id), // ← ambil dari useParams
        data: formData, // ← ini adalah FormData, bisa kamu ubah di mutationFn
      },
      {
        onSuccess: () => {
            ...
            <form onSubmit={storeUpdateMail}>
              <div className="grid grid-cols-12 items-center gap-4 mb-4">
                ...
                <input
                  ...
                  className="col-span-5  border border-gray-300 rounded-md py-2 px-3  text-base focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <div className="grid grid-cols-12 items-center gap-4 mb-4">
                ...
                <textarea
                  ...
                  className="col-span-5 border  border-gray-300 rounded-md py-2 px-3 text-black text-base resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              {mail?.note != "" && (
                <div>
                  ...
                  <p className="col-span-5 border border-gray-300 rounded-md py-2 px-3 text-black text-base focus:outline-none focus:ring-2 focus:ring-blue-500">
                    {mail?.note}
                  </p>
                </div>
              )}
              ...
              >
                <Link to="/dvpersuratan/suratmasuk">Batal</Link>
              </button>
            </form>
          </div>
        </div>
